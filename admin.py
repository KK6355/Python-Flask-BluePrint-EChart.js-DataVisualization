...
def approveStudent():
    ...
    emailSender.sendEmail(email, f"Kia ora {name}, we are happy to advise your application to register with Lincoln University's Postgraduate Monitoring System has been approved.")
...
def rejectStudent():
    ...
    emailSender.sendEmail(
        email, f"Kia ora {name}, Unfortunately your request to register with Lincoln University's Postgraduate Monitoring System has been rejected as the details you provided did not match our records.")
    return redirect(url_for('admin.students'))
    ...
    for student in studentList:
        if student['StudentID'] not in suspendedStuId:
            ...
            for i in range(0,orderDiff):
                ...
                if (not dep and not period and not dep and not repStatus and not studentSearch ) or (dep==student['DepartmentCode'] and repStatus not in ['Completed','Uncomplete'])  or repStatus=='Norecord':
                    reportsWithStatus.append(
                        {
